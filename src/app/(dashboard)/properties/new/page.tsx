...
export default function NewPropertyPage() {
  ...
  return (
    <div className="max-w-lg">
      ...
      <p className="text-gray-500 mt-1 mb-6">Enter an NYC address to look up the building and start monitoring.</p>
      ...
      {scanResult && newPropertyId && !scanning && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-6 mb-4">
          ...
          <div className="flex gap-3 mt-4">
            ...
            <Link
              href="/properties"
              className="border border-gray-200 text-gray-700 px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50 active:scale-[0.97] transition-all duration-200"
            >
              All Properties
            </Link>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className={`flex-1 px-4 py-2.5 border border-gray-200 rounded-xl bg-white text-gray-900 placeholder:text-gray-400 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all duration-200 min-h-[44px] ${addressError ? 'border-red-500' : ''}`}
                onKeyDown={e => e.key === 'Enter' && handleResolve()}
              />
              <button
                onClick={handleResolve}
                disabled={resolving || address.length < 5}
                className="bg-indigo-600 text-white px-4 py-2.5 rounded-xl font-medium hover:bg-indigo-700 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-sm hover:shadow-md whitespace-nowrap"
              >
          ...
          {duplicateId && resolved && (
            <div className="bg-yellow-50 border border-yellow-300 rounded-2xl p-4 space-y-2">
              ...
              <Link
                ...
                className="inline-block mt-2 bg-yellow-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-yellow-700 active:scale-[0.97] transition-all duration-200"
              >
          ...
          {resolved && !duplicateId && (
            <div className="bg-green-50 border border-green-200 rounded-2xl p-4 space-y-2">
              ...
              <button
                ...
                className="mt-2 bg-indigo-600 text-white px-4 py-2.5 rounded-xl font-medium hover:bg-indigo-700 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-sm hover:shadow-md"
              >
